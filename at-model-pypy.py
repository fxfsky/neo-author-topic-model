		...
		for di in xrange(0, self._D):
			...
			for wi in xrange(0, len(doc)):
				...
				#z = n.random.choice(self._K, 1)[0]
                                z = n.random.randint(self._K)
				#random sampling author
				#a = n.random.choice(len(authors),1)[0]
                                a = n.random.randint(len(authors))
			...
			for di in xrange(0, len(self._docList)):
				...
				for wi in xrange(0, len(doc)):
					...
					#idx = [i for i,x in enumerate(sample) if x==1][0]
                                        idx = bisect(n.cumsum(pdf), n.random.rand())
					...
					self.author_assigned[di][wi] = new_a
                        ...
                        timeEnd = time.time()
                        print 'Elapsed time for single interation : ' + str(timeEnd - timeBegin)
...
if __name__ == '__main__':
	...
	print 'Initializing...'
	atm = at_model(bow, 100, len(authors), d_wi, paperid_authorid.values())
	print 'Inference started'
	atm_results = atm.sampling_topics(100)
        cPickle.dump(atm_results, open('atm_results.pkl', 'wb'))
